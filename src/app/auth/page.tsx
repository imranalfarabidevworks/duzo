...
export default function AuthPage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--dark)',display:'flex',alignItems:'center',justifyContent:'center',padding:24,position:'relative'}}>
      <div style={{position:'absolute',inset:0,backgroundImage:`url('https://images.unsplash.com/photo-1610832958506-aa56368176cf?w=1400&q=70&auto=format&fit=crop')`,backgroundSize:'cover',backgroundPosition:'center',filter:'brightness(0.18)'}}/>
        ...
        <div style={{background:'rgba(255,255,255,0.06)',backdropFilter:'blur(24px)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:24,padding:'40px 36px'}}>
          <div style={{textAlign:'center',marginBottom:32}}>
            ...
            <p style={{color:'var(--text-s)',fontSize:14}}>
              {mode === 'login' ? 'Sign in to your Duzo account' : 'Join the Duzo community today'}
            </p>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',background:'rgba(255,255,255,0.04)',borderRadius:12,padding:4,marginBottom:28}}>
            {(['login','register'] as const).map(m=>(
              <button key={m} onClick={()=>{setMode(m);setError('');}} style={{padding:'10px',borderRadius:9,border:'none',background:mode===m?'linear-gradient(135deg,#c8922a,#e0a93a)':'transparent',color:mode===m?'#fff':'rgba(255,255,255,0.5)',fontSize:14,fontWeight:600,cursor:'pointer',transition:'all 0.2s'}}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{marginBottom:16}}>
              <label style={{fontSize:13,color:'var(--text-s)',display:'block',marginBottom:6}}>Email Address</label>
            ...
            <div style={{marginBottom:24,position:'relative'}}>
              ...
              <button type="button" onClick={()=>setShowPw(!showPw)} style={{position:'absolute',right:14,top:35,background:'none',border:'none',color:'var(--text-m)',cursor:'pointer'}}>
                {showPw ? <EyeOff size={16}/> : <Eye size={16}/>}
              </button>
            </div>

            {error && (
              <div style={{background:'rgba(255,80,80,0.1)',border:'1px solid rgba(255,80,80,0.2)',borderRadius:8,padding:'10px 14px',color:'#ff6b6b',fontSize:13,marginBottom:16}}>
                {error}
              </div>
            )}

            <button type="submit" disabled={loading} className="btn-gold" style={{width:'100%',justifyContent:'center',padding:'15px',fontSize:16,opacity:loading?0.7:1}}>
          ...
          <p style={{textAlign:'center',color:'var(--text-m)',fontSize:13,marginTop:20}}>
            ...
            <button onClick={()=>{setMode(mode==='login'?'register':'login');setError('');}} style={{background:'none',border:'none',color:'var(--gold)',cursor:'pointer',fontSize:13,fontWeight:600}}>
              {mode === 'login' ? 'Create one' : 'Sign in'}
            </button>
          </p>

          {mode === 'login' && (
            <div style={{marginTop:20,padding:'12px 14px',background:'rgba(200,146,42,0.06)',border:'1px solid rgba(200,146,42,0.15)',borderRadius:10,fontSize:12,color:'var(--text-s)',lineHeight:1.6}}>
              <strong style={{color:'var(--gold)'}}>Demo:</strong> Register a new account, then login. Orders are saved to MongoDB.
            </div>
          )}
        </div>
      </div>
